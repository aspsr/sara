        ...
        <div class="col">
            <div class="card border-dark mb-3">
                ...
                <div class="col-md-12" style="text-align: center">
                       Stai prenotando per il giorno &nbsp;<strong>{{date("d/m/Y", strtotime($dataView['data'])) }}</strong>&nbsp; nel centro vaccinale di &nbsp;<strong>{{\App\Models\CentroVaccinale::where("id", $dataView['centroVaccinale'])->first()->descrizione}}</strong>.
                    </div>
                            ...
                            @foreach($dataView['fasce']->chunk(3) as $fasce)
                                ...
                                @foreach($fasce as $fascia => $prenotazioni)
                                    <td class="text-center">
                                        <form method="post" action="{{ route('cambia')}}">
                                            @csrf
                ...
                <form method="post" action="{{ route('tornaIndietro')}}">
                    @csrf
                    <input type="hidden" name="patientID" value="{{$dataView['patient']->id}}">
                    <button type="submit" class="btn btn-primary "><i class="bi bi-arrow-return-left"></i>&nbsp;&nbsp;Torna indietro</button>
                </form>
            </div>
        </div>
    </div>
